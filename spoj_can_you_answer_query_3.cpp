...
ar<int,4>combine( ar<int,4>A , ar<int,4>B )
{
    int a , b , c , d ;
...
ar< int , 4 > max( vector< ar< int , 4 >>&T , int N , int a , int b )
{
   ar< int , 4 > lt = { INT_MIN , INT_MIN , INT_MIN , INT_MIN } , rt = { INT_MIN , INT_MIN , INT_MIN , INT_MIN } ;

   for( a+=N,b+=N ; a < b ; a >>= 1 , b >>= 1 )
   {
      if(a&1) lt = combine( lt , T[a++] );
      if(b&1) rt = combine( T[--b], rt );
   }

   return combine( lt , rt );
}

void add( vector< ar<int,4> >&T , int N , int p , int x )
{
    for( T[p+=N] ={x,x,x,x} ; p>>=1 ; )
...
void go()
{
   ...
   vector< ar< int , 4 > >T( 2*N , { 0 , 0 , 0 , 0 } );

   for( int i = 0 ; i < N ; i++ )
   T[i+N] = { A[i] , A[i] , A[i] , A[i] };
