...
//--------------------------------template ends here ----------------------------------//
      ...
      int gcd( vector<int>&T , int N , int a , int b )
      {  
         int res = 0;
         
         for( a += N , b += N , res = T[a] ;  a < b ; a >>=1 , b>>=1 )
         {
             if(a&1) res = __gcd( res , T[a++] );
             if(b&1) res = __gcd( res , T[--b] );
         }

         return res ;
      }
      
      ar< int , 2 > combine( ar< int, 2 > A , ar< int , 2 > B )
      {
         if( A[0] == B[0] )
            return { A[0] , A[1] + B[1] } ;
      ...
      ar<int,2> qry( vector<ar<int,2>>&T , int N , int a , int b )
      {
         ar< int , 2 > P = { 0 , 0 } ;

         for( a += N , b += N , P[0] = T[a][0] ; a < b ; a >>= 1 , b>>=1 )
         {
            if(a&1) P = combine( P , T[a++] );
            if(b&1) P = combine( T[--b] , P );
         }
       ...
       void go()
       {
           ...
           for( int i = N-1 ; i > 0 ; i-- )
            T1[i] = __gcd( T1[2*i] , T1[2*i +1 ] );


           vector< ar<int, 2 > >T2( 2*N , { 0 , 0 } );

           for( int i = 0 ; i < N ; i++ )
            T2[i+N] = { A[i] , 1 };

           for( int i = N-1 ; i > 0 ; i-- )
            T2[i] = combine( T2[2*i] , T2[2*i+1] );
