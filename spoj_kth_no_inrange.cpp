...
//--------------------------------template ends here ----------------------------------//
    ...
    int kth( vector<vector<int>>&T , int N , int a , int b , int m )
    {
        int cnt = 0 ;
        int P = 0 ;
        ...
        for( a += N , b += N ; a < b ; a >>= 1 , b >>= 1 )
        {
            ...
            if(a&1)
            {
                cnt += upper_bound( T[a].begin() , T[a].end() , m ) - T[a].begin() ;
                a++;
            }
            ...
            {
                b--;
                cnt  += upper_bound( T[b].begin() , T[b].end() , m ) - T[b].begin();
            }
       ...
       void go()
       {
           ...
           vector< vector< int > > T( 2*N , vector<int>{} );
