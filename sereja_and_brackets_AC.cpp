...
//--------------------------------template ends here ----------------------------------//
   ...
   // c - ()
       ...
       ar< int , 3 > combine( ar<int ,3>A , ar<int,3>B )
       {
         int t = min( A[0] , B[1] );
         return { A[0] + B[0] - t , A[1] + B[1] - t , A[2] + B[2] +2*t };
       }

       ar< int , 3 > sum( vector< ar<int,3>> &T , int N , int a , int b )
       {
          ar< int , 3 > P = { 0 , 0 , 0 };
          auto Q = P ;
