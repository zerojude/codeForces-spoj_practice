...
// 0 - corresponds to OR 
// 1 - corresponds to XOR 

       
       void add( vector<ar<int,2>> &T , int N , int p , int x )
       {
           for( T[p+N][0] = x , T[p+N][1] = x , p += N ; p > 1 ; p >>= 1 )
           {
                T[p>>1][0] = T[p][1] | T[p^1][1] ;
                T[p>>1][1] = T[p][0] ^ T[p^1][0] ;
           }
       }
       ...
       void go()
       {
            ...
            for( int i = N-1 ; i > 0 ; i-- )
            {
                T[i][0] = T[2*i][1] | T[2*i + 1 ][1] ;    // OR of XORs 
                T[i][1] = T[2*i][0] ^ T[2*i + 1 ][0] ;    // XOR of ORs 
            }
